...
public static class ReferenceCodeGenerator
{
        ...
        string @namespace)
    {
        ...
        var buildName = GetBuildMethodName(schema);
        ...
        using (cw.Block($"namespace {@namespace}"))
        {
            using (cw.Block($"public sealed class {className}"))
            {
                ...
                using (cw.Block($"public static {className} FromRow(RefTableSchema schema, RefTableRow row)"))
                {
                    foreach (var column in schema.Columns)
                    {
                        var propName = ToIdentifier(column.Name);
                    ...
                    cw.WriteLine();
                    cw.WriteLine($"return new {className}");
                    using (cw.Block(""))
                    {
            ...
            using (cw.Block($"public sealed partial class {nameof(ReferenceTableManager)}"))
            {
                ...
                using (cw.Block($"private void {buildName}()"))
                {
                    ...
                    cw.WriteLine("foreach (var row in table.Rows)");
                    using (cw.Block(""))
                    {
                        cw.WriteLine($"var obj = {className}.FromRow(schema, row);");
                        cw.WriteLine($"{dicName}[obj.{keyPropName}] = obj;");
                    }
                }
            
                cw.WriteLine();

                var getterName = $"Get{className.Substring(3)}";
                using (cw.Block($"public {className}? {getterName}({keyTypeName} id)"))
                {
