...
본 프로젝트는 상용 엔터프라이즈 솔루션 릴리즈를 목적으로 하기보다는, 대규모 동시 접속 환경에서 발생하는 로우레벨 최적화 이슈와 동시성 제어 기술을 검증하기 위해 설계된 **상용 서비스 수준의 아키텍처를 지향하는 포트폴리오/토이 프로젝트**입니다. 

서버의 가용성과 처리량(Throughput)을 제한하는 두 가지 최대 요인인 **GC(Garbage Collection) 스파이크**와 **스레드 간 락 경합(Lock Contention)**을 프레임워크 레벨에서 통제하고 우회하는 것을 최우선 사상으로 구현되었습니다.

---

## 🚀 Key Features

SP.Engine은 복잡한 로우레벨 제어(스레드 동기화, GC 메모리 관리, 네트워크 예외 처리)를 프레임워크 런타임으로 철저히 은닉했습니다. 콘텐츠 개발자는 오직 게임 비즈니스 로직 구현에만 집중할 수 있습니다.

* **동기화(Lock) 오버헤드가 없는 비즈니스 런타임**
  * 복잡한 스레드 제어와 컨텍스트 스위칭 오버헤드를 프레임워크 런타임 깊숙이 은닉했습니다.
  * 다수의 클라이언트 요청을 코어 수에 맞춘 논리 스레드(Fiber) 큐에 안전하게 분배하므로, 개발자는 멀티스레드 환경의 고질적인 데드락(Deadlock)이나 `lock` 경합에 대한 피로감 없이 단일 스레드 환경처럼 직관적으로 로직에 집중할 수 있습니다.
    
* **런타임 오버헤드가 없는 Zero-Reflection 직렬화**
  * 성능 저하와 GC 스파이크의 주범인 C# 기본 리플렉션과 박싱(Boxing) 연산을 배제했습니다.
  * 프레임워크 초기화 시점에 컴파일된 동적 IL(Intermediate Language)과 정적 메타데이터를 활용하여, 개발자가 패킷 구조체만 정의하면 즉시 초고속으로 압축 및 직렬화 통신이 가능합니다.
    
* **상태 유실 없는 무중단 세션 복구 (Seamless Resumption)**
  * Wi-Fi와 LTE/5G 전환이 잦은 모바일 네트워크 환경에 특화된 방어 로직을 제공합니다.
  * 물리적인 소켓 단선이 발생해도 논리적인 유저 세션과 송신 윈도우(Sliding Window)를 즉시 파괴하지 않고 동결합니다. 재접속 시 유실된 패킷 구간만 정밀하게 재전송하여 게임 플레이 경험의 단절을 막습니다.

* **목적형 하이브리드 네트워크 스택 (TCP / UDP)**
  * 데이터의 절대적 무결성이 필요한 로직(인증, 결제, 인벤토리 등)은 TCP 채널로, 약간의 유실을 감수하더라도 실시간 레이턴시가 최우선인 로직(이동 및 시야 동기화, 타격 액션)은 UDP 채널로 분리하여 전송할 수 있습니다.
  * UDP 채널의 경우 자체적인 패킷 파편화 조립(Fragment Assembly) 기능을 내장하여 가변 MTU 환경에서도 대용량 데이터를 안전하게 처리합니다.

* **분산 아키텍처 확장을 위한 S2S (Server-to-Server) 통신**
  * 단일 물리 서버의 한계를 넘어, 전역 매치메이킹이나 분산 백엔드 노드와 유기적으로 연동할 수 있는 커넥터 인프라를 내장하고 있습니다.
  * 서버 간 통신에도 클라이언트와 동일한 고성능 스택(Zero-Allocation 직렬화 등)을 대칭적으로 적용하여 대용량 상태 동기화를 고속으로 처리합니다.

* **도메인 격리를 위한 플러그인 모듈 (Pluggable Modules)**
  * 코어 네트워크 엔진과 게임 비즈니스 로직을 완벽하게 분리하여, 메인 서버 루프에 병목을 주지 않는 독립형 모듈 시스템을 제공합니다.
  * **`SP.Database`**: 비동기 DB 커넥션 관리 및 자체 초경량 마이크로 ORM 인프라
  * **`SP.Rank`**: 청크 분할(Chunk-based) 알고리즘이 적용된 초고속 인메모리 기반 실시간 리더보드 시스템
  * **`SP.Resource`**: CodeGen 기반의 게임 정적 데이터(테이블) 파싱, 리소스 압축 해제 및 다국어(Localization) 관리 시스템
